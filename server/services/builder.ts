...
`;
  ...
  private async buildDOCX(
    ...
    if (isReport) {
      return this.buildDOCXReport(taskId, title, slides, sources);
    }
  ...
  private async buildDOCXReport(
    ...
    await logger.stepStart(taskId, "Building structured DOCX report");
    
    // ─── Source Quality Filtering ───
    const BLACKLIST_DOMAINS = [
      'microsoft.com/create', 'scribd.com', 'geeksforgeeks', 'ivypanda', 'researchgate',
      'calculatorgenius', 'calculatorsoup', 'gradecalculator', 'slideshare'
    ];
    const filteredSources = sources.filter(url => {
      const lower = url.toLowerCase();
      return !BLACKLIST_DOMAINS.some(domain => lower.includes(domain));
    });
    ...
    const sanitizeText = (text: string): string => {
      if (!text) return "";
      let clean = text
        // Remove meta phrases entirely
        .replace(/\b(this slide|this presentation|template|microsoft create|will include|will describe|will adopt|will provide)\b/gi, '')
        // Rewrite future tense to present/past
        ...
        .replace(/\bwill provide\b/gi, 'provides')
        .replace(/\bwill explain\b/gi, 'explains')
        // Clean up whitespace
        .replace(/\s{2,}/g, ' ')
        .replace(/\s+\./g, '.')
        .replace(/\s+,/g, ',')
    ...
    const buildTextFromSlide = (slide?: SlideIn): string => {
      ...
      if (Array.isArray(c.bullets)) {
        parts.push(c.bullets.map(b => sanitizeText(b)).filter(Boolean).join('. '));
      }
      return parts.join(' ').replace(/\s{2,}/g, ' ').trim();
    };
    
    const execSummary = buildTextFromSlide(execSummarySlide) || "This report presents comprehensive analysis and findings based on thorough research and data evaluation.";
    const introduction = buildTextFromSlide(introSlide) || "This section provides background context and establishes the foundation for the analysis presented in this report.";
    const methods = buildTextFromSlide(methodsSlide) || "Research methodology included systematic data collection, analysis, and validation to ensure accuracy and reliability of findings.";
    const results = resultsSlides.map(buildTextFromSlide).filter(Boolean).join(' ') || "Analysis revealed key patterns and significant insights detailed in the following sections.";
    const discussion = buildTextFromSlide(discussionSlide) || "These findings have important implications for understanding the subject matter and inform future directions for research and application.";
    ...
    children.push(
      ...
      new Paragraph({ text: execSummary }),
      new Paragraph({ text: "" }),
    );
    ...
    children.push(
      ...
      new Paragraph({ text: introduction }),
      new Paragraph({ text: "" }),
    );
    ...
    // Embed charts from all slides with robust error handling
    for (const slide of slides) {
      if (embeddedChartsCount >= maxCharts) break;
      ...
      if (chartUrl) {
        try {
          const response = await axios.get(chartUrl, {
            responseType: "arraybuffer",
            timeout: 15000,
            headers: { "User-Agent": "Agent Diaz/1.0 (docx-report)" },
            maxRedirects: 3,
          });
          ...
          children.push(
            new Paragraph({
              children: [
                new TextRun({ text: `Figure ${embeddedChartsCount + 1}: ${sanitizeText(c.chart?.title || spec?.title || "Data Visualization")}`, bold: true }),
              ],
            }),
            new Paragraph({
              children: [
          ...
          chartFailures.push(errMsg);
          await logger.trace(taskId, `Chart embed failed: ${errMsg}`);
          
          // Fallback: Add textual description instead of silently failing
          children.push(
            new Paragraph({
              children: [
                new TextRun({ 
                  text: `[Visualization: ${sanitizeText(c.chart?.title || spec?.title || "Data chart")} - data available in source materials]`, 
                  italics: true, 
                  color: "666666" 
                }),
              ],
    ...
    // Log chart embedding status
    if (chartFailures.length > 0) {
      await logger.trace(taskId, `Chart embedding: ${embeddedChartsCount} succeeded, ${chartFailures.length} failed`);
    }
    
    // ─── Acceptance Guard: Ensure Quality Standards ───
    const isDataAnalysis = title.toLowerCase().match(/\b(analysis|data|performance|metrics|statistics)\b/) ||
                          results.toLowerCase().match(/\b(\d+%|\d+\/\d+|percentage|rate|comparison)\b/);
    
    if (isDataAnalysis && embeddedChartsCount === 0) {
      await logger.trace(taskId, `QUALITY WARNING: Data analysis report has zero embedded charts. Adding analysis note.`);
      
      // Add explicit note about missing visualizations
      children.push(
        new Paragraph({ text: "Data Visualization Note", heading: HeadingLevel.HEADING_2 }),
        new Paragraph({ 
          children: [
            new TextRun({ 
              text: "This report contains quantitative analysis. Visualizations could not be embedded due to technical constraints, but the numerical findings and statistical analysis remain valid. Consult source materials for graphical representations.", 
              italics: true, 
              color: "666666" 
            })
          ]
    ...
    const totalContentLength = execSummary.length + introduction.length + methods.length + results.length + discussion.length;
    if (totalContentLength < 500) {
      throw new Error("Report content too sparse - unable to generate meaningful analysis from provided data");
    }
    
    // Discussion
    children.push(
      new Paragraph({ text: "Discussion", heading: HeadingLevel.HEADING_1 }),
      new Paragraph({ text: discussion }),
      new Paragraph({ text: "" }),
    );
    ...
    if (filteredSources.length > 0) {
      ...
      filteredSources.slice(0, 20).forEach((url, i) => {
        children.push(
          new Paragraph({ children: [new TextRun({ text: `[${i + 1}] ${url}`, size: 20 })] }),
        );
      });
    } else {
      await logger.trace(taskId, "WARNING: Report has no valid references (all blacklisted)");
    }
    ...
    await logger.trace(taskId, `Report complete: ${totalContentLength} chars, ${embeddedChartsCount} charts, ${filteredSources.length} refs`);
    await logger.stepEnd(taskId, "Building structured DOCX report");
