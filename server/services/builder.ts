...
`;
  ...
  // ─── Validation Gates: Pre-export quality checks ───
  private async validateReportArtifacts(
    ...
    if (totalLength < 500) {
      errors.push(`Total content too short: ${totalLength} chars (minimum 500)`);
    }
    
    // Chart/table requirement for data analysis
    const isDataAnalysis = allText.match(/\b(analysis|data|performance|metrics|statistics)\b/);
    if (isDataAnalysis && chartCount === 0 && !hasTable) {
      errors.push('Data analysis report must contain at least 1 chart or table');
    }
    
    // Citation requirement
    if (allCitations.size < 3) {
      errors.push(`Insufficient citations: ${allCitations.size} found (minimum 3 required)`);
    }
    
    // Source vetting
    if (sources.length < 3) {
      errors.push(`Insufficient sources: ${sources.length} (minimum 3 required)`);
    }
